defmodule PC.Form do
    ...
    <%= if field_has_errors?(assigns) do %>
      <div class={@class}>
        <%= for translated_error <- @translated_errors do %>
          <div
            class="mt-1 text-xs italic text-danger-500 invalid-feedback"
  ...
  defp label_classes(assigns) do
    ...
    "#{if field_has_errors?(assigns), do: "has-error", else: ""} #{type_classes} #{assigns[:class] || ""} block mb-2 text-sm font-medium text-gray-900 dark:text-gray-200"
  end

  defp text_input_classes(has_error) do
    "#{if has_error, do: "has-error", else: ""} block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:focus:border-primary-500 sm:text-sm disabled:bg-gray-100 disabled:cursor-not-allowed dark:bg-gray-800 dark:text-gray-300 dark:disabled:bg-gray-700 focus:outline-none"
  end

  defp select_classes(has_error) do
    "#{if has_error, do: "has-error", else: ""} block w-full py-2 pl-3 pr-10 text-base border-gray-300 rounded-md cursor-pointer focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:focus:border-primary-500 disabled:bg-gray-100 disabled:cursor-not-allowed focus:outline-none sm:text-sm dark:disabled:bg-gray-700 dark:text-gray-300 dark:bg-gray-800"
  end

  defp file_input_classes(has_error) do
    "#{if has_error, do: "has-error", else: ""} text-sm rounded-md cursor-pointer focus:outline-none file:border-0 text-slate-500 file:text-primary-700 file:font-semibold file:px-4 file:py-2 file:mr-6 file:rounded-md hover:file:bg-primary-100 file:bg-primary-200 dark:file:bg-primary-300 hover:dark:file:bg-primary-200"
  end

  defp color_input_classes(has_error) do
    "#{if has_error, do: "has-error", else: ""} border-gray-300 cursor-pointer focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:focus:border-primary-500"
  end

  defp range_input_classes(has_error) do
    "#{if has_error, do: "has-error", else: ""} w-full border-gray-300 cursor-pointer focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:focus:border-primary-500"
  end

  defp checkbox_classes(has_error) do
    "#{if has_error, do: "has-error", else: ""} w-5 h-5 transition-all duration-150 ease-linear border-gray-300 rounded cursor-pointer text-primary-700 dark:bg-gray-800 dark:border-gray-600 disabled:bg-gray-300 dark:disabled:bg-gray-600 disabled:cursor-not-allowed"
  end

  defp checkbox_group_layout_classes(assigns) do
    case assigns[:layout] do
      :row ->
        "flex flex-row gap-4 mb-3"

      _col ->
        "flex flex-col gap-4 mb-3"
    end
  end
  ...
  defp radio_classes(has_error) do
    "#{if has_error, do: "has-error", else: ""} w-4 h-4 border-gray-300 cursor-pointer text-primary-600 focus:ring-primary-500 dark:bg-gray-800 dark:border-gray-600"
  end
