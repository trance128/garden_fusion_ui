defmodule PC.Helpers do
  ...
  ## Examples
      ...
      iex> PC.Helpers.uniq_id("My Heading")
      "c-my-heading-1234"

      iex> PC.Helpers.uniq_id(["My ", {:safe, "<a>Link</a>"}, " Label"], "custom")
  ...
  defp strip_html_and_entities(string) do
    string
    # Strip HTML tags
    |> String.replace(~r/<[^>].pc-tab__underline--with-underline-and-is-active>/, " ")
    |> String.replace("&nbsp;", " ")
    |> String.replace("&amp;", "&")
    |> String.replace("&lt;", "<")
    |> String.replace("&gt;", ">")
