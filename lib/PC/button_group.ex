defmodule PC.ButtonGroup do
  ...
  Note: Phoenix LiveView >= 0.20.17 is required to prevent arbitrary attributes like phx-.pc-tab__underline--with-underline-and-is-active
  ...
  defp group_btn_class(assigns) do
    base_classes = [
      ...
      "whitespace-nowrap focus:ring-gray-200 focus:z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  ...
  defp position_class(idx, last_idx) do
    cond do
      idx == 0 and idx == last_idx -> "rounded-md"
      idx == 0 -> "rounded-r-none rounded-l-md"
      idx == last_idx -> "rounded-l-none rounded-r-md"
      true -> "rounded-none"
    end
  end

  defp size_class(size), do: "inline-flex items-center justify-center--#{size}"

  defp font_weight_class("font-normal"), do: "font-normal"
  defp font_weight_class("font-medium"), do: "font-medium"
  defp font_weight_class("font-semibold"), do: "font-semibold"
  defp font_weight_class("font-bold"), do: "font-bold"
  defp font_weight_class(custom_class), do: custom_class

  defp background_class(nil), do: "bg-white hover:bg-gray-100 dark:bg-gray-900 dark:hover:bg-gray-800 text-gray-800 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-300"
  defp background_class(custom_class), do: custom_class

  defp border_class(nil), do: "border border-gray-200 dark:border-gray-800"
