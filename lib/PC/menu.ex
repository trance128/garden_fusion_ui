defmodule PC.Menu do
  ...
  defp menu_icon_classes(is_active),
    do:
      "flex-shrink-0 w-5 h-5 flex-shrink-0 w-5 h-5--#{if is_active, do: "active", else: "inactive"}"

  defp menu_item_base_classes(),
    do: "flex items-center w-full gap-3 px-3 py-2 text-sm font-semibold leading-none transition duration-200 rounded-md"

  # Active state
  defp menu_item_classes(page, page),
    do: "#{menu_item_base_classes()} group text-primary-700 dark:text-primary-400 bg-gray-50 dark:bg-gray-800"

  # Inactive state
  defp menu_item_classes(_current_page, _link_page),
    do: "#{menu_item_base_classes()} group text-gray-700 hover:bg-gray-50 dark:text-gray-200 hover:text-gray-900 dark:hover:text-white dark:hover:bg-gray-700"
